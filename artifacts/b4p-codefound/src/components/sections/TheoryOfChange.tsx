...
export function TheoryOfChange({ blueBackground = false }: { blueBackground?: boolean }) {
  const accentClass = blueBackground ? 'text-accent' : 'text-secondary';

  return (
    <section className={`py-24 md:py-32 ${blueBackground ? 'bg-primary' : 'bg-[#062e37]'} text-white relative overflow-hidden`}>
      <div className={`absolute top-0 right-0 w-[800px] h-[800px] ${blueBackground ? 'bg-white/10' : 'bg-secondary/20'} rounded-full blur-[120px] -translate-y-1/2 translate-x-1/3 opacity-50 pointer-events-none`} />
      <div className={`absolute bottom-0 left-0 w-[600px] h-[600px] ${blueBackground ? 'bg-accent/20' : 'bg-primary/20'} rounded-full blur-[100px] translate-y-1/3 -translate-x-1/3 opacity-50 pointer-events-none`} />

      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24 items-center">
          
          <div className="lg:col-span-7">
            <span className={`text-sm font-bold tracking-[0.2em] ${accentClass} uppercase mb-6 block`}>
              Our Philosophy
            </span>
            ...
            <div className={`flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 md:gap-6 ${blueBackground ? 'bg-white/10 border-white/20' : 'bg-white/5 border-white/10'} p-6 md:p-8 rounded-2xl border backdrop-blur-sm`}>
              <div className="flex-1 text-left md:text-center">
                <span className={`block ${accentClass} text-sm font-bold tracking-widest uppercase mb-2`}>Step 1</span>
              ...
              <div className="flex-1 text-left md:text-center">
                <span className={`block ${accentClass} text-sm font-bold tracking-widest uppercase mb-2`}>Step 2</span>
              ...
              <div className="flex-1 text-left md:text-center">
                <span className={`block ${accentClass} text-sm font-bold tracking-widest uppercase mb-2`}>Step 3</span>
